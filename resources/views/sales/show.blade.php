                                ...
                                <table class="table table-sm table-borderless">
                                    <tr>
                                        ...
                                        <td class="text-end fw-bold">Rs.
                                            {{ number_format($sale->total_amount - ($sale->transport_cost ?? 0) + $sale->discount_amount, 2) }}
                                        </td>
                                    </tr>
                                    ...
                                    <tr class="border-top">
                                        ...
                                        <td class="text-end h5 fw-bold text-primary">Rs.
                                            {{ number_format($sale->total_amount, 2) }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-end fw-bold text-muted">Paid:</td>
                                        <td class="text-end text-success">Rs. {{ number_format($sale->paid_amount, 2) }}
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="text-end fw-bold text-muted">Balance Due:</td>
                                        <td class="text-end text-danger">Rs.
                                            {{ number_format(max(0, $sale->total_amount - $sale->paid_amount), 2) }}</td>
                                    </tr>
                                </table>
                            </div>
                        </div>
                                    ...
                                    <table class="table table-sm table-bordered">
                                        ...
                                        <tbody>
                                            @foreach($sale->payments as $payment)
                                                <tr>
                                                    <td>{{ \Carbon\Carbon::parse($payment->payment_date)->format('Y-m-d H:i') }}
                                                    </td>
                ...
                <div class="card border-0 shadow-sm rounded-4 mb-3">
                    <div class="card-body p-4">
                        ...
                        <div class="mb-3">
                            @if($sale->status == 'paid')
                                <div
                                    class="p-2 bg-success-subtle text-success text-center rounded fw-bold border border-success-subtle">
                                    Fully Paid</div>
                            @elseif($sale->status == 'partial')
                                <div
                                    class="p-2 bg-warning-subtle text-warning text-center rounded fw-bold border border-warning-subtle">
                                    Partially Paid</div>
                            @else
                                <div
            ...
            <div class="border-bottom border-2 border-dark pb-2 mb-2">
                <div class="row align-items-center">
                    <div class="col-4">
                        ...
                        <div
                            ...
                            @if(!empty($globalSettings['company_logo']))
                                <img src="{{ asset($globalSettings['company_logo']) }}" alt="Logo"
                                    style="max-width: 100%; max-height: 100%;">
                            @else
                                <div
                                    style="border: 2px solid #000; width: 100%; height: 100%; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 1.2rem;">
                                    LOGO
                                </div>
                            @endif
                        </div>
                    ...
                    <div class="col-8 text-end">
                        <h2 class="fw-bold text-uppercase mb-1" style="color: #000080;">
                            {{ $globalSettings['company_name'] ?? config('app.name') }}</h2>
                        <p class="mb-0 small fw-bold" style="white-space: pre-line;">
                            {{ $globalSettings['company_address'] ?? '' }}</p>
                ...
                <table class="table table-bordered border-dark mb-0" style="font-size: 0.9rem;">
                    ...
                    <tbody>
                        @foreach($sale->items as $item)
                            <tr>
                                ...
                                <td class="p-1">
                                    {{ $item->product->name }}
                                    @if($item->description) <br><small
                                    class="text-muted fst-italic">({{ $item->description }})</small> @endif
                                </td>
                    ...
                    <tfoot>
                            ...
                            <td colspan="3" rowspan="4" class="align-top p-2" style="border-right: 1px solid #000;">
                                <div class="d-flex gap-3 mb-2 small fw-bold">
                                    <div class="d-flex align-items-center">
                                        <div style="width:15px; height:15px; border:1px solid #000; position: relative;"
                                            class="me-1">
                                            @if($sale->total_amount - $sale->paid_amount <= 0 && $sale->payment_method == 'cash')
                                                <i
                                                    class="fa-solid fa-check small position-absolute top-50 start-50 translate-middle"></i>
                                            @endif
                                        </div> Cash
                                    </div>
                                    <div class="d-flex align-items-center">
                        ...
                        <tr>
                            ...
                            <td class="text-end fw-bold p-1 h5 mb-0">
                                {{ number_format($sale->total_amount - $sale->paid_amount, 2) }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
                    ...
                    <div class="modal-body">
                        ...
                        <div id="modalChequeFields" class="d-none border rounded p-3 bg-light mb-3">
                            ...
                            <div class="mb-2">
                                ...
                                <input type="text" class="form-control form-control-sm" name="cheque_number" maxlength="6"
