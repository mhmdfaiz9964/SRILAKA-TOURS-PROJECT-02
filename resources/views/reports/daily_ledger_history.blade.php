...
<div class="container-fluid py-4">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        ...
        <div class="d-flex gap-2">
            <a href="{{ route('reports.daily-ledger') }}" class="btn btn-primary btn-sm px-4 rounded-3 d-flex align-items-center gap-2" style="background: #6366f1; border: none;">
            ...
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-end shadow-lg border-0 rounded-4 p-2 mt-2" style="min-width: 180px;">
                    <a class="dropdown-item d-flex align-items-center gap-3 p-2 rounded-3" href="{{ route('reports.daily-ledger.history', array_merge(request()->all(), ['export' => 'excel'])) }}">
                        <div class="bg-success bg-opacity-10 p-2 rounded-circle">
                            <i class="fa-solid fa-file-excel text-success"></i>
                        </div>
                        <span class="small fw-bold">Excel Format</span>
                    </a>
                    <a class="dropdown-item d-flex align-items-center gap-3 p-2 rounded-3 mt-1" href="{{ route('reports.daily-ledger.history', array_merge(request()->all(), ['export' => 'pdf'])) }}">
    ...
    <div class="bg-white rounded-4 shadow-sm border border-light mb-4">
        <div class="p-3 bg-light bg-opacity-10">
            <form action="{{ route('reports.daily-ledger.history') }}" method="GET" class="row g-2 align-items-center">
                <div class="col-md-3">
                    <input type="date" name="from_date" value="{{ request('from_date') }}" class="form-control form-control-sm border-light rounded-3 shadow-none" placeholder="From Date">
                </div>
                <div class="col-md-3">
                    <input type="date" name="to_date" value="{{ request('to_date') }}" class="form-control form-control-sm border-light rounded-3 shadow-none" placeholder="To Date">
                </div>
                <div class="col-md-2">
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="ps-4 fw-medium text-dark border-light">{{ \Carbon\Carbon::parse($record->date)->format('Y-m-d') }}</td>
                        <td class="text-end fw-bold text-success border-light">{{ number_format($record->total_income, 2) }}</td>
                        <td class="text-end fw-bold text-danger border-light">{{ number_format($record->total_expense, 2) }}</td>
                        ...
                        <td class="text-end pe-4 border-light">
                            <button class="btn btn-sm btn-icon border-0 text-primary shadow-none" onclick="viewDetails('{{ $record->date }}')">
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content rounded-4 border-0 shadow-lg" style="height: 80vh;">
            ...
            <div class="modal-footer border-top-0">
                <a href="#" id="exportPdfBtn" class="btn btn-sm btn-outline-danger rounded-3 gap-2 d-flex align-items-center">
                    <i class="fa-solid fa-file-pdf"></i> Download PDF
                </a>
                <a href="#" id="editBtn" class="btn btn-sm btn-primary rounded-3 px-4" style="background: #6366f1; border: none;">
                    Edit This Day
                </a>
            </div>
        </div>
    ...
    function viewDetails(date) {
        fetch(`/reports/daily-ledger/details/${date}`)
            ...
            .then(data => {
                ...
                // Update Links
                document.getElementById('editBtn').href = `/reports/daily-ledger?date=${data.date}`;
