                ...
                <div class="card border-0 shadow-sm" style="border-radius: 12px;">
                    <div class="card-body p-4">
                        <form action="{{ route('cheques.update', $cheque) }}" method="POST">
                            ...
                            <div class="row g-4">
                                <div class="col-md-6">
                                    <label class="form-label fw-bold small text-muted text-uppercase">Cheque Number (6
                                        Digits)</label>
                                    <input type="text" name="cheque_number" id="cheque_number_input"
                                        class="form-control border-light shadow-none @error('cheque_number') is-invalid @enderror"
                                        value="{{ old('cheque_number', $cheque->cheque_number) }}" required maxlength="6"
                                        minlength="6">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="date" name="cheque_date"
                                        class="form-control border-light shadow-none @error('cheque_date') is-invalid @enderror"
                                        value="{{ old('cheque_date', $cheque->cheque_date ? \Carbon\Carbon::parse($cheque->cheque_date)->format('Y-m-d') : '') }}"
                                        required>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="bank_id"
                                        class="form-select border-light shadow-none @error('bank_id') is-invalid @enderror"
                                        required>
                                        @foreach($banks as $bank)
                                            <option value="{{ $bank->id }}" {{ $cheque->bank_id == $bank->id ? 'selected' : '' }}>
                                                {{ $bank->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="number" step="0.01" name="amount"
                                        class="form-control border-light shadow-none" value="{{ $cheque->amount }}"
                                        required>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label fw-bold small text-muted text-uppercase">Payer Name / Client
                                        Name</label>
                                    <input type="text" name="payer_name" class="form-control border-light shadow-none"
                                        value="{{ $cheque->payer_name }}" required>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label fw-bold small text-muted text-uppercase">3rd Part</label>
                                    <input type="text" name="payee_name" class="form-control border-light shadow-none"
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="return_reason" class="form-select border-light shadow-none">
                                        ...
                                        <option value="Customer" {{ $cheque->return_reason == 'Customer' ? 'selected' : '' }}>
                                            Customer</option>
                                    </select>
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <select name="payment_status" class="form-select border-light shadow-none">
                                        <option value="pending" {{ $cheque->payment_status == 'pending' ? 'selected' : '' }}>
                                            Pending</option>
                                        <option value="partial paid" {{ $cheque->payment_status == 'partial paid' ? 'selected' : '' }}>Partial Paid</option>
                                        <option value="paid" {{ $cheque->payment_status == 'paid' ? 'selected' : '' }}>Paid
                                        </option>
                                    </select>
                                </div>
                                <div class="col-12">
                                    <label class="form-label fw-bold small text-muted text-uppercase">Notes</label>
                                    <textarea name="notes" class="form-control border-light shadow-none"
                                        rows="3">{{ $cheque->notes }}</textarea>
                                </div>
                            </div>

                            <div class="mt-4 pt-3 border-top d-flex gap-2">
                                <button type="submit" class="btn btn-primary px-4 shadow-sm"
                                    style="background: #6366f1; border: none; border-radius: 8px;">Update Cheque</button>
                                <a href="{{ route('cheques.index') }}" class="btn btn-light px-4 border-light"
    ...
    <script>
        document.getElementById('cheque_number_input').addEventListener('input', function (e) {
            ...
            if (e.target.value.length === 6) {
                ...
                e.target.classList.remove('border-danger', 'border-warning');
                e.target.classList.add('border-success');
            } else if (e.target.value.length > 0) {
                msg.textContent = 'Need ' + (6 - e.target.value.length) + ' more digits...';
