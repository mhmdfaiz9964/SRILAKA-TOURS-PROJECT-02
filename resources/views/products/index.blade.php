                        ...
                        <div class="card-body p-3">
                            <div class="d-flex align-items-center justify-content-between mb-2">
                                ...
                                <div class="rounded-circle d-flex align-items-center justify-content-center"
                                    style="width: 32px; height: 32px; background: #fff; color: #10b981;">
                                    <i class="fa-solid fa-coins"></i>
                                </div>
                            </div>
                            <div class="h4 fw-bold mb-0" style="color: #047857;">LKR {{ number_format($totalCostValue, 2) }}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="card-header bg-white border-bottom-0 py-3 px-4">
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
                    <form action="{{ route('products.index') }}" method="GET"
                        ...
                        <select class="form-select form-select-sm" style="width: 150px;" name="is_main_product"
                        ...
                        <select class="form-select form-select-sm" style="width: 150px;" name="sort"
                            onchange="this.form.submit()">
                            ...
                            <option value="lowest_price" {{ request('sort') == 'lowest_price' ? 'selected' : '' }}>Lowest
                                Price</option>
                            <option value="highest_stock" {{ request('sort') == 'highest_stock' ? 'selected' : '' }}>Highest
                                Stock</option>
                            <option value="lowest_stock" {{ request('sort') == 'lowest_stock' ? 'selected' : '' }}>Lowest
                                Stock</option>
                        </select>
                        ...
                        <a href="{{ route('products.index') }}" class="btn btn-light btn-sm"><i
                        ...
                        <tbody>
                            @forelse($products as $product)
                                <tr>
                                            ...
                                            <div class="d-flex align-items-center gap-2">
                                                ...
                                                <div>
                                                    @if($product->parentProduct)
                                                        <div class="text-muted" style="font-size: 0.7rem;">
                                                            <i
                                                                class="fa-solid fa-arrow-turn-up fa-rotate-90 me-1"></i>{{ $product->parentProduct->name }}
                                                        </div>
                                                    @endif
                                                    <div class="fw-bold text-dark">{{ $product->name }}</div>
                                                </div>
                                    ...
                                    <td class="small">
                                        <span class="badge rounded-pill border border-0 px-3 py-1"
                                            style="background-color: #581c87; color: #fff;">
                                    ...
                                    <td class="small fw-bold text-success">
                                        {{ number_format($product->current_stock * $product->cost_price, 2) }}</td>
