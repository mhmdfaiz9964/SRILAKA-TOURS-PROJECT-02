    ...
    <div class="container-fluid py-4">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4">
            ...
            @can('in-cheque-create')
                <a href="{{ route('in-cheques.create') }}" class="btn btn-primary px-4 rounded-3 shadow-sm"
                ...
                <form action="{{ route('in-cheques.index') }}" method="GET" class="row g-3 align-items-end">
                    ...
                    <div class="col-md-2">
                        ...
                        <div class="position-relative">
                            ...
                            <input type="text" name="search" value="{{ request('search') }}"
                                class="form-control form-control-sm ps-4 border-light rounded-3" placeholder="Name or #...">
                        </div>
                    </div>
                    <div class="col-md-2">
                        ...
                        <select class="form-select form-select-sm border-light rounded-3" name="sort">
                            ...
                            <option value="highest_amount" {{ request('sort') == 'highest_amount' ? 'selected' : '' }}>Highest
                                Amount</option>
                            <option value="lowest_amount" {{ request('sort') == 'lowest_amount' ? 'selected' : '' }}>Lowest
                                Amount</option>
                            <option value="name_az" {{ request('sort') == 'name_az' ? 'selected' : '' }}>Payer Name (A-Z)
                            </option>
                        </select>
                    </div>
                    <div class="col-md-2">
                        ...
                        <select name="payer_name" class="form-select form-select-sm border-light rounded-3">
                            ...
                            @foreach($payers as $payer)
                                <option value="{{ $payer }}" {{ request('payer_name') == $payer ? 'selected' : '' }}>{{ $payer }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-1">
                        ...
                        <select name="status" class="form-select form-select-sm border-light rounded-3">
                            ...
                            <option value="deposited" {{ request('status') == 'deposited' ? 'selected' : '' }}>Deposited
                            </option>
                    ...
                    <div class="col-md-2">
                        ...
                        <input type="text" id="daterange" class="form-control form-control-sm border-light rounded-3"
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <div class="dropdown d-inline-block">
                                ...
                                <ul class="dropdown-menu shadow-lg border-0 rounded-4 p-2 mt-2">
                                    <li>
                                        <button type="submit" formaction="{{ route('in-cheques.export') }}" name="export"
                                            value="excel"
        ...
        <div class="table-responsive">
            <form id="bulkUpdateForm" action="{{ route('cheques.bulk-update-combined') }}" method="POST">
                ...
                <div class="p-2 border-bottom bg-light d-flex gap-2 align-items-center" id="bulkActions"
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <select name="status" class="form-select form-select-sm" style="width: 160px;" id="bulkStatusSelect"
                            required>
                        ...
                        <div id="transferTypeContainer" class="d-none align-items-center gap-2">
                            <select name="transfer_type" id="transferTypeSelect" class="form-select form-select-sm"
                                style="width: 130px;">
                        ...
                        <div id="thirdPartyContainer" class="d-none align-items-center gap-2">
                            <select name="third_party_name" id="bulkThirdPartyName" class="form-select form-select-sm"
                                style="width: 200px;">
                            ...
                            <button type="button" class="btn btn-outline-secondary btn-sm" data-bs-toggle="modal"
                                data-bs-target="#createThirdPartyModal" title="Add New Third Party">
                        ...
                        <div id="supplierContainer" class="d-none align-items-center gap-2">
                            <select name="supplier_id" id="bulkSupplierId" class="form-select form-select-sm"
                                style="width: 200px;">
                    ...
                    <span class="ms-auto small text-muted fw-bold"><span id="selectedCount" class="text-primary">0</span>
                        records selected</span>
                </div>
                    ...
                    <tbody>
                        @forelse($cheques as $cheque)
                            <tr>
                                ...
                                @endphp
                                <td class="ps-4">
                                    <input type="checkbox" name="cheque_ids[]" value="{{ $cheque->id }}" class="cheque-checkbox"
                                        {{ $isLocked ? 'disabled title="Locked cheques cannot be updated"' : '' }}>
                                </td>
                                <td>
                                    <span class="badge rounded-pill px-2 py-1"
                                        style="background: #eff6ff; color: #3b82f6; font-size: 0.65rem;">
                                        IN
                                    </span>
                                </td>
                                <td class="small text-muted">{{ \Carbon\Carbon::parse($cheque->cheque_date)->format('d/m/Y') }}
                                </td>
                                ...
                                <td class="text-end pe-4">
                                    <div class="d-flex justify-content-end gap-1">
                                        @can('in-cheque-edit')
                                            @if(!$isLocked)
                                                <a href="{{ route('in-cheques.edit', $in_cheque = $cheque) }}"
                                                    class="btn btn-sm btn-icon border-0 text-dark shadow-none">
                                        ...
                                        @can('in-cheque-delete')
                                            <button type="button" class="btn btn-sm btn-icon border-0 text-danger shadow-none"
                                                onclick="confirmDeleteCheque('{{ route('in-cheques.destroy', $cheque) }}')">
