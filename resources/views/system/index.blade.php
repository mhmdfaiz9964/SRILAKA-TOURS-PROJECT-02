            ...
            <div class="d-flex gap-2">
                @can('system-manage')
                    <form action="{{ route('system.storage-link') }}" method="POST">
                        @csrf
                        <button type="submit"
                            class="btn btn-white btn-sm px-3 shadow-sm border-light d-flex align-items-center gap-2">
                            <i class="fa-solid fa-link text-black"></i> Storage Link
                        </button>
                    </form>
                    <button type="button" onclick="confirmBackup()"
                        class="btn btn-warning btn-sm px-4 shadow-sm d-flex align-items-center gap-2 text-dark fw-bold">
                        <i class="fa-solid fa-database"></i> Backup DB
                    </button>
                    <form id="backup-form" action="{{ route('system.backup') }}" method="POST" style="display: none;">
                        @csrf
                    </form>
        ...
        <script>
            function confirmBackup() {
                Swal.fire({
                    ...
                    text: "This will generate a SQL dump of your database and download it. It will also be stored securely on the server storage.",
                    icon: 'info',
                ...
                <div class="card border-0 shadow-sm rounded-4 mb-4">
                    <div class="card-body p-4 text-center">
                        <div class="avatar-circle mx-auto mb-3"
                            style="width: 80px; height: 80px; background: #f5f3ff; color: #6366f1; font-size: 2rem;">
                        ...
                        <div class="timeline-container ps-3 border-start">
                            @forelse($logs as $log)
                                <div class="timeline-item position-relative mb-5 ps-4">
                                    <div class="timeline-dot position-absolute rounded-circle"
                                        style="left: -9px; top: 0; width: 18px; height: 18px; background: #fff; border: 4px solid {{ $loop->first ? '#6366f1' : '#e2e8f0' }}; z-index: 1;">
                                    </div>

                                    <div class="d-flex align-items-center justify-content-between mb-2">
                                        <h6 class="fw-bold mb-0">{{ $log->title }} <span
                                                class="badge bg-light text-primary border ms-2"
                                                style="font-size: 0.65rem;">{{ $log->version }}</span></h6>
                                        <span
                                            class="extra-small text-muted">{{ $log->created_at->format('M d, Y • H:i') }}</span>
                                    </div>
                                    <p class="small text-muted mb-0">{{ $log->description }}</p>
                                    @if($loop->first)
                                        <div class="mt-2">
                                            <span
                                                class="badge rounded-pill bg-success bg-opacity-10 text-success py-1 px-3 border-0"
                                                style="font-size: 0.65rem;">Current Active Build</span>
                                        </div>
                                    @endif
                                </div>
                            @empty
