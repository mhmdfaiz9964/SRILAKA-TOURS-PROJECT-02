                ...
                <div class="d-flex align-items-center gap-2">
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-end shadow-lg border-0 rounded-4 p-2 mt-2"
                            ...
                            <a class="dropdown-item d-flex align-items-center gap-3 p-2 rounded-3"
                                href="{{ route('customers.index', array_merge(request()->query(), ['export' => 'excel'])) }}">
                            ...
                            <a class="dropdown-item d-flex align-items-center gap-3 p-2 rounded-3 mt-1"
                                href="{{ route('customers.index', array_merge(request()->query(), ['export' => 'pdf'])) }}">
                    ...
                    @can('customer-create')
                        <a href="{{ route('customers.create') }}"
                            ...
                            style="background: #6366f1; border: none; padding: 0.6rem 1rem;">
        ...
        <div class="row mb-4">
            <div class="col-md-4">
                <a href="{{ request('outstanding_only') == '1' ? route('customers.index', request()->except('outstanding_only')) : route('customers.index', array_merge(request()->query(), ['outstanding_only' => 1])) }}"
                    ...
                    <div class="card border-0 shadow-sm rounded-4 transition-all {{ request('outstanding_only') == '1' ? 'ring-2 ring-primary' : '' }}"
                        style="background: #fff7ed;">
                        <div class="card-body p-3">
                            <div class="d-flex align-items-center justify-content-between mb-2">
                                <span class="text-muted small fw-bold text-uppercase"
                                    style="color: #9a3412 !important;">Total Outstanding</span>
                                <div class="rounded-circle d-flex align-items-center justify-content-center shadow-sm"
                                    style="width: 32px; height: 32px; background: #fff; color: #f97316;">
                                    <i class="fa-solid fa-file-invoice-dollar"></i>
                                </div>
                            </div>
                            <div class="h4 fw-bold mb-0" style="color: #c2410c;">LKR
                                {{ number_format($totalOutstanding, 2) }}</div>
                            <div class="small mt-1 text-muted" style="font-size:0.75rem;">
                                @if(request('outstanding_only'))
                                    <span class="text-danger fw-bold"><i class="fa-solid fa-filter me-1"></i> Filter
                                        Active</span>
                                @else
        ...
        <div class="card border-0 shadow-sm" style="border-radius: 12px; overflow: hidden;">
            <div class="p-3 border-bottom bg-light bg-opacity-10">
                <form action="{{ route('customers.index') }}" method="GET" class="row g-3 align-items-end">
                    ...
                    <div class="col-12">
                        <div class="p-2 px-3 small fw-bold text-muted border-top pt-3">{{ $customers->total() }} Results
                        </div>
                    </div>
                </form>
            </div>
                        ...
                        <tbody>
                            @forelse($customers as $customer)
                                <tr class="cursor-pointer"
                                    ...
                                    <td class="small">
                                        <span
                                            class="badge {{ $customer->outstanding >= ($customer->credit_limit ?? 0) ? 'bg-danger' : 'bg-success' }} rounded-pill">
                                            {{ number_format($customer->credit_limit, 2) }}
                                        </span>
                                    </td>
                                    <td
                                        class="small fw-bold {{ $customer->outstanding >= ($customer->credit_limit ?? 0) ? 'text-danger' : 'text-success' }}">
    ...
    <script>
        function confirmDelete(id, formId) {
            Swal.fire({
                ...
                text: "Are you sure you want to delete this customer? All their history will be preserved but they will be removed from the list.",
                icon: 'warning',
