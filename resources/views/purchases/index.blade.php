                ...
                <form action="{{ route('purchases.index') }}" method="GET" class="row g-3 align-items-end">
                    ...
                    <div class="col-md-2">
                        ...
                        <select name="sort" class="form-select form-select-sm border-light rounded-3">
                            ...
                            <option value="lowest_amount" {{ request('sort') == 'lowest_amount' ? 'selected' : '' }}>Lowest
                                Amount</option>
                            <option value="name_az" {{ request('sort') == 'name_az' ? 'selected' : '' }}>Supplier (A-Z)
                            </option>
                        </select>
                    </div>
                    <div class="col-md-2">
                    ...
                    <div class="col-12">
                        <div
                            ...
                            <span>{{ $purchases->total() }} Results</span>
                            <div class="d-flex align-items-center gap-2">
                        ...
                        <tbody>
                            @forelse($purchases as $purchase)
                                <tr class="cursor-pointer"
                                    onclick="window.location='{{ route('purchases.show', $purchase->id) }}'">
                                    ...
                                    <td class="small">
                                        <span
                                            class="badge bg-{{ $purchase->purchase_type == 'import' ? 'info' : 'secondary' }}-subtle text-{{ $purchase->purchase_type == 'import' ? 'info' : 'secondary' }} rounded-pill border-0 text-uppercase"
                                    ...
                                    <td
                                        class="small fw-bold {{ ($purchase->total_amount - $purchase->paid_amount) > 0 ? 'text-danger' : 'text-muted' }}">
                                    ...
                                    <td class="text-end pe-4" onclick="event.stopPropagation();">
                                        <div class="d-flex justify-content-end gap-1">
                                            ...
                                            @endcan
                                            @can('purchase-delete')
                                                <button type="button" class="btn btn-sm btn-icon border-0 text-danger"
                                                    onclick="confirmDelete({{ $purchase->id }}, 'delete-purchase-{{ $purchase->id }}')">
                                                    <i class="fa-regular fa-trash-can"></i>
                                                </button>
                                                <form id="delete-purchase-{{ $purchase->id }}"
